...
The code is available under the _docker_ folder and consists of a Docker Compose file (_docker-compose.yml_), a Dockerfile (_nignx/Dockerfile_) and an NGINX configuration file (_nginx/ninx.conf_), the latest o build the NGINX proxy image:
```
docker/
├── docker-compose.yml
└── nginx
